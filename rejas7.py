...
# --- CONFIGURACIÓN ---
st.set_page_config(page_title="Rejas Pro v3.1", page_icon="⚒️")
...
# --- SIDEBAR: COSTOS Y CONFIGURACIÓN ---
st.sidebar.header("⚙️ Costos y Configuración")
p_barra_ref = st.sidebar.number_input("Precio Barra 40x40 (6m)", value=32000)
p_kg = p_barra_ref / (1.91 * 6)
p_m2_malla = st.sidebar.number_input("Precio m2 Malla", value=13500)
p_litro_pintura = st.sidebar.number_input("Precio Litro Pintura", value=14500)
p_consumibles = st.sidebar.number_input("Gasto Consumibles (% Mat.)", 1, 15, 5) / 100

st.sidebar.divider()
metodo_mo = st.sidebar.selectbox("Método de Mano de Obra", ["% de Materiales", "Por Día / Jornal", "Por Metro Cuadrado"])
if metodo_mo == "% de Materiales": val_mo = st.sidebar.number_input("% sobre materiales", 50, 200, 100)
elif metodo_mo == "Por Día / Jornal": val_mo = st.sidebar.number_input("Precio por Día", 20000, 100000, 35000)
else: val_mo = st.sidebar.number_input("Precio por m2", 10000, 80000, 25000)
...
st.title("🛡️ Calculador Pro v3.1")

# --- VARIABLES GLOBALES ---
m_barrotes = 0; m_planchuela = 0; listado_caños = {}; m2_malla_total = 0; m_angulo_interno = 0
costo_herrajes = 0; detalles_obra = []; lista_herrajes_txt = []; m_angulo_guia_porton = 0
mat_barrote_elegido = "1/2 (12.7mm)"; area_total_m2 = 0
...
if st.checkbox("Incluir Paños de Frente", value=True, key="chk_f"):
    ...
    with c2:
        ...
        if st.checkbox("¿Lleva Bastidor de Caño?", value=True, key="bas_f"):
...
if st.checkbox("Incluir Otros Paños", key="chk_v"):
    ...
    with c2v:
        ...
        if st.checkbox("¿Lleva Bastidor de Caño?", value=True, key="bas_v"):
        ...
        if estilo_v == "Barrotes":
            ...
            m_planchuela += ancho_v * st.number_input("Filas Planchuela", 2, 6, 2, key="pl_v") * cant_v
        else:
...
if st.checkbox("Incluir Puerta", key="chk_p"):
    c1p, c2p = st.columns(2)
    with c1p:
        ancho_p = st.number_input("Ancho", 0.5, 2.0, 0.9, key="w_p")
        alto_p = st.number_input("Alto", 1.5, 3.0, 2.0, key="h_p")
        area_total_m2 += (ancho_p * alto_p)
    with c2p:
        estilo_p = st.selectbox("Estilo", ["Barrotes", "Malla/Desplegado"], key="st_p")
        ...
        if estilo_p == "Barrotes":
            ...
            m_planchuela += ancho_p * st.number_input("Filas Planchuela", 2, 6, 2, key="pl_p")
        else:
            m2_malla_total += (ancho_p * alto_p)
            m_angulo_interno += (ancho_p + alto_p) * 2
        costo_herrajes += sum([PRECIOS_HERRAJES["Cerradura Seg."], PRECIOS_HERRAJES["Pomelas (Par)"], PRECIOS_HERRAJES["Caja Cerradura"]])
        lista_herrajes_txt.extend(["Cerradura Seg.", "Pomelas", "Caja Cerradura"])
...
if st.checkbox("Incluir Portón", key="chk_port"):
    ...
    with c1port:
        ...
        area_total_m2 += (ancho_port * 2.1)
    with c2port:
        estilo_v = st.selectbox("Estilo", ["Barrotes", "Malla/Desplegado"], key="st_port")
        ...
        if estilo_v == "Barrotes":
            m_barrotes += (math.ceil(ancho_port / 0.12) + 1) * 2.1
            m_planchuela += ancho_port * st.number_input("Filas Planchuela", 2, 6, 2, key="pl_port")
        else:
            m2_malla_total += (ancho_port * 2.1)
            m_angulo_interno += (ancho_port + 2.1) * 2
        ...
        if tipo_v == "Corredizo":
            ...
            if auto: costo_herrajes += (ancho_port * PRECIOS_HERRAJES["Cremallera (m)"])
...
if st.checkbox("¿Lleva Postes Adicionales?", key="chk_postes"):
    ...
    detalles_obra.append(f"{cant_postes} Postes {m_c_poste}")

# --- 6. FLETE Y COLOCACIÓN ---
st.divider()
st.header("6. Servicios")
col_f, col_c = st.columns(2)
with col_f:
...
if st.button("🚀 GENERAR PRESUPUESTO COMPLETO", type="primary", use_container_width=True):
    ...
    l_pint = math.ceil(sup_pint / 10 * 2)
    costo_p = l_pint * p_litro_pintura

    # Materiales Base
    p_b = PESOS_H.get(mat_barrote_elegido, 1.21)
    c_caños = sum([m * PESOS_C[med] * p_kg for med, m in listado_caños.items()])
    c_mats_puros = c_caños + (m_angulo_interno + m_angulo_guia_porton) * PESO_ANGULO_MARCO * p_kg + (m_barrotes * p_b * p_kg) + (m_planchuela * 0.9 * p_kg) + (m2_malla_total * p_m2_malla) + costo_herrajes + costo_p
    
    # Consumibles
    c_consumibles = c_mats_puros * p_consumibles
    c_mats_total = (c_mats_puros + c_consumibles) * (1 + perc_desp)

    # Mano de Obra
    if metodo_mo == "% de Materiales": mo_final = c_mats_total * (val_mo / 100)
    elif metodo_mo == "Por Día / Jornal": 
        dias = st.number_input("Días de trabajo", 1, 30, 3)
        mo_final = dias * val_mo
    else: mo_final = area_total_m2 * val_mo
    
    mo_final *= dict_calidad[nivel_mo]

    # Totales
    total_final = redon(c_mats_total + mo_final + (0 if bonif_flete else p_flete) + (0 if bonif_coloc else p_coloc))

    # --- PANTALLA ---
    st.success(f"## TOTAL: ${total_final:,.0f}")
    
    # Detalles para el Cliente
    txt_cliente = f"PRESUPUESTO HERRERÍA\n" + "-"*30 + f"\nTrabajo: {', '.join(detalles_obra)}\n"
    txt_cliente += f"Terminación: {nivel_mo}\n"
    txt_cliente += f"Flete: {'$0 (BONIFICADO)' if bonif_flete else f'${p_flete:,.0f}'}\n"
    txt_cliente += f"Colocación: {'$0 (BONIFICADO)' if bonif_coloc else f'${p_coloc:,.0f}'}\n"
    txt_cliente += f"\nTOTAL A PAGAR: ${total_final:,.0f}"

    st.subheader("📋 Resumen para el Cliente")
    st.info(txt_cliente)

    # Lista de Materiales (Taller)
    txt_taller = f"LISTA DE MATERIALES\n" + "="*30 + "\n"
    for med, met in listado_caños.items(): txt_taller += f"- {med}: {math.ceil((met*1.1)/6)} barras\n"
    if m_barrotes > 0: txt_taller += f"- Barrotes {mat_barrote_elegido}: {math.ceil((m_barrotes*1.1)/6)} barras\n"
    if m_planchuela > 0: txt_taller += f"- Planchuela: {math.ceil((m_planchuela*1.1)/6)} barras\n"
    if m2_malla_total > 0: txt_taller += f"- Malla: {m2_malla_total:.2f} m2\n"
    txt_taller += f"- Pintura: {l_pint} Litros\n- Consumibles: Est. ${c_consumibles:,.0f}"

    with st.expander("🛠️ Ver Lista de Taller"):
        st.text(txt_taller)
    
    st.markdown(f'[📩 Enviar por WhatsApp]({gen_ws(txt_cliente)})')
>>>>>>> d420561 (Version 4.4 completa con dependencias)
